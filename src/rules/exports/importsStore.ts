...
function storeImportDeclaration(file: FileInfo, dependency: DependencyInfo, node: ImportDeclaration) {
...
function getImportDeclaration(file: FileInfo, specifierName: string): ExtendedImportInfo | null {
  const fileImports = importsCache.load(file.path);
  return (fileImports && fileImports[specifierName]) ?? null;
}

export { storeImportDeclaration, getImportDeclaration };
